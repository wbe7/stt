...
const GROQ_DEFAULT_EDIT_CONFIG: EditConfig = {
...
export async function editTextGroq(
  text: string,
  options?: Partial<EditConfig & EditRequest>
): Promise<EditResponseWrapper> {
  const config = { ...GROQ_DEFAULT_EDIT_CONFIG, ...options }
  ...
  try {
    ...
    const client = new OpenRouterClient('groq', config.apiKey, config.customBaseUrl)
