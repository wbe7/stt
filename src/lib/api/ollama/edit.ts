...
import { DEFAULT_EDIT_CONFIG } from '../openrouter/edit-types'
import type {
  EditResponseWrapper,
  EditConfig,
  EditRequest,
  Message,
} from '../openrouter/edit-types'

export async function editTextOllama(
  text: string,
  options?: Partial<EditConfig & EditRequest>
): Promise<EditResponseWrapper> {
  const config = { ...DEFAULT_EDIT_CONFIG, ...options, provider: 'ollama' as const }
  ...
  try {
    ...
    const client = new OpenRouterClient(config.provider)
